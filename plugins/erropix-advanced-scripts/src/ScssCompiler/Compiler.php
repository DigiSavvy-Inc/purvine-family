...
class Compiler
{
    ...
    /**
     * @var string|\ERROPiX\AdvancedScripts\ScssCompiler\Formatter
     */
     ...
     * Compile directive
     *
     * @param \ERROPiX\AdvancedScripts\ScssCompiler\Block|array $directive
    ...
    protected function compileCommentValue($value, $pushEnv = false)
    {
        ...
        if (isset($value[2])) {
            ...
            try {
                ...
                $this->logger->warn('Ignoring interpolation errors in multiline comments is deprecated and will be removed in SCSS 2.0. ' . $this->addLocationToMessage($e->getMessage()), true);
                // ignore error in comment compilation which are only interpolation
            } catch (SassException $e) {
                $this->logger->warn('Ignoring interpolation errors in multiline comments is deprecated and will be removed in SCSS 2.0. ' . $e->getMessage(), true);
                // ignore error in comment compilation which are only interpolation
            }
...
EOL;
    ...
    public function registerFunction($name, $callback, $argumentDeclaration = null)
    {
        if (self::isNativeFunction($name)) {
            @trigger_error(sprintf('The "%s" function is a core sass function. Overriding it with a custom implementation through "%s" is deprecated and won\'t be supported in SCSS 2.0 anymore.', $name, __METHOD__), E_USER_DEPRECATED);
        }

        if ($argumentDeclaration === null) {
            @trigger_error('Omitting the argument declaration when registering custom function is deprecated and won\'t be supported in SCSS 2.0 anymore.', E_USER_DEPRECATED);
        }
            ...
            foreach ($this->importPaths as $dir) {
                ...
                if (\is_callable($dir)) {
                    ...
                    if (!\is_null($file)) {
                        ...
                        @trigger_error(sprintf('Returning a file to import for CSS or external references in custom importer callables is deprecated and will not be supported anymore in SCSS 2.0. This behavior is not compliant with the Sass specification. Update your "%s" importer.', $callableDescription), E_USER_DEPRECATED);
    ...
    public function setEncoding($encoding)
    {
        if (!$encoding || strtolower($encoding) === 'utf-8') {
            ...
            @trigger_error(sprintf('The "%s" method is deprecated. Parsing will only support UTF-8 in SCSS 2.0. The non-UTF-8 parsing of SCSS 1.x is not spec compliant.', __METHOD__), E_USER_DEPRECATED);
        }
...
will be an error in future versions of Sass.\n         on line $line of $fname";
    ...
    {
        @trigger_error(sprintf('The "scssphp-glob" function is deprecated an will be removed in SCSS 2.0. Register your own alternative through "%s::registerFunction', __CLASS__), E_USER_DEPRECATED);

        $this->logger->warn('The "scssphp-glob" function is deprecated an will be removed in SCSS 2.0.', true);
