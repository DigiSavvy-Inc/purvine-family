...
class Kadence_Custom_Fonts {
	...
	public function on_plugins_loaded() {
		...
		define( 'KT_CUSTOM_FONTS_VERSION', '1.1.3' );
	...
	public function on_init() {
		...
		add_action( 'admin_enqueue_scripts', array( $this, 'admin_typekit_custom_font_css' ), 100 );
				...
				if ( 'adobe' == $type ) {
					...
					if ( is_array( $typekit ) && isset( $typekit['families'] ) && is_array( $typekit['families'] ) ) {
						foreach ( $typekit['families'] as $font_slug => $font_data ) {
							if ( isset( $k_font_array[ $font_slug ] ) ) {
								if ( isset( $font_data['weights'] ) && is_array( $font_data['weights'] ) ) {
									foreach ( $font_data['weights'] as $weight ) {
										if ( ! isset( $k_font_array[ $font_slug ]['weights'][ $weight ] ) ) {
											$k_font_array[ $font_slug ]['weights'][ $weight ] = $weight;
										}
									}
								}
							} else {
								$k_font_array[ $font_slug ] = array(
		...
		if ( $k_fonts ) {
			foreach ( $k_fonts as $font ) {
				...
				if ( 'adobe' == $type ) {
					...
					if ( is_array( $typekit ) && isset( $typekit['families'] ) && is_array( $typekit['families'] ) ) {
						foreach ( $typekit['families'] as $font_slug => $font_data ) {
							$k_font_array[ $font_slug ] = 'kadence-custom-fonts';
						}
					}
				} else {
					$name = get_post_meta( $font->ID, '_kad_font_name', true );
	...
	public function add_block_fonts() {
		...
		if ( $fonts ) {
			...
			wp_enqueue_script( 'kadence_custom_fonts_block_scripts', KT_CUSTOM_FONTS_URL . 'assets/js/custom_fonts_blocks.js', array( 'wp-blocks', 'wp-i18n', 'wp-element', 'wp-editor', 'wp-api' ), KT_CUSTOM_FONTS_VERSION, true );
		...
		if ( $fonts ) {
			foreach ( $fonts as $font ) {
				...
				if ( 'adobe' == $type ) {
					...
					if ( is_array( $typekit ) && isset( $typekit['families'] ) && is_array( $typekit['families'] ) ) {
						foreach ( $typekit['families'] as $font_slug => $font_data ) {
							if ( isset( $font_array['font_familes'][ $font_slug ] ) ) {
								if ( isset( $font_data['variations'] ) && is_array( $font_data['variations'] ) ) {
									foreach ( $font_data['variations'] as $variation ) {
										if ( ! in_array( $variation, $font_array['font_familes'][ $font_slug ]['styles'] ) ) {
											$font_array['font_familes'][ $font_slug ]['styles'][ $variation ] = $variation;
										}
									}
								}
							} else {
								$font_array['font_familes'][ $font_slug ] = array(
									'name' => $font_slug,
									'styles' => array(),
								);
								if ( isset( $font_data['variations'] ) && is_array( $font_data['variations'] ) ) {
	...
	public function admin_typekit_custom_font_css() {
		$fonts = $this->kt_font_query();
		$wp_styles = wp_styles();
		$style     = $wp_styles->query( 'wp-edit-blocks', 'registered' );
		if ( $fonts ) {
			foreach ( $fonts as $font ) {
				$type = get_post_meta( $font->ID, '_kad_font_type', true );
				if ( $type == 'adobe' ) {
			...
			foreach ( $fonts as $font ) {
				...
				if ( 'adobe' == $type ) {
					...
					if ( is_array( $typekit ) && isset( $typekit['families'] ) && is_array( $typekit['families'] ) ) {
						foreach ( $typekit['families'] as $font_slug => $font_data ) {
							if ( ! empty( $font_slug ) ) {
								if ( isset( $custom_fonts[ $font_slug ] ) ) {
									if ( isset( $font_data['variations'] ) && is_array( $font_data['variations'] ) ) {
										foreach ( $font_data['variations'] as $variation ) {
											if ( ! in_array( $variation, $custom_fonts[ $font_slug ]['v'] ) ) {
												$custom_fonts[ $font_slug ]['v'][] = $variation;
											}
										}
									}
								} else {
