...
class Kadence_Blocks_Pro_Dynamic_Content {
			...
			if ( isset( $block['attrs'] ) && is_array( $block['attrs'] ) ) {
				...
				if ( isset( $blockattr['kadenceDynamic'] ) && is_array( $blockattr['kadenceDynamic'] ) && isset( $blockattr['kadenceDynamic']['images'] ) && is_array( $blockattr['kadenceDynamic']['images'] ) && isset( $blockattr['kadenceDynamic']['images']['enable'] ) && $blockattr['kadenceDynamic']['images']['enable'] ) {
					...
					if ( $update ) {
						...
						$lightbox_cap = ( ! empty( $blockattr['lightboxCaption'] ) && $blockattr['lightboxCaption'] ? true : false );
	...
	public function render_gallery_images( $image, $attributes ) {
		...
		$padding_bottom = '';
		if ( ( 'masonry' === $type || 'fluidcarousel' === $type ) && ! empty( $image['width'] ) && ! empty( $image['height'] ) ) {
		...
		$output .= '<figure class="' . esc_attr( implode( ' ', $fig_classes ) ) . '">';
		if ( ! empty( $href ) ) {
			$output .= '<a href="' . esc_url( $href ) . '" class="kb-gallery-item-link" ' . ( ( $link_to === 'custom' && '_blank' === $link_target ) || ( $link_to === 'media' && $lightbox === 'new_tab' ) ? 'target="_blank"' : '' ) . ' ' . ( ( $link_to === 'custom' && '_blank' === $link_target ) || ( $link_to === 'media' && $lightbox === 'new_tab' ) ? 'rel="noopener noreferrer"' : '' ) . '>';
		}
		$output .= '<div class="kb-gal-image-radius" ' . ( ! empty( $padding_bottom ) ? 'style="max-width:' . $image['width'] . 'px;"' : '' ) . '>';
		$output .= $img;
	...
	public function get_field_content( $args, $post = null ) {
		...
		if ( 'core' === $origin ) {
			...
			if ( self::POST_GROUP === $group ) {
				...
				if ( $post && is_object( $post ) && 'publish' === $post->post_status && apply_filters( 'kadence_dynamic_enable_password_content', empty( $post->post_password ) ) && post_type_supports( $post_type_obj->name, 'author' ) ) {
