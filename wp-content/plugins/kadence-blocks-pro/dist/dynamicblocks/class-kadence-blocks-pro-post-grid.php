...
class Kadence_Blocks_Pro_Post_Grid {
	...
	public function register_scripts() {
		...
		wp_register_style( 'kadence-splide', KBP_URL . 'dist/assets/css/kadence-splide.min.css', array(), KBP_VERSION );
		wp_register_script( 'kadence-slick', KBP_URL . 'dist/vendor/slick.min.js', array( 'jquery' ), KBP_VERSION, true );
		wp_register_script( 'kadence-splide', KBP_URL . 'dist/assets/js/splide.min.js', array(), KBP_VERSION, true );
		wp_register_script( 'kadence-splide-auto-scroll', KBP_URL . 'dist/assets/js/splide-auto-scroll.min.js', array( 'kadence-splide' ), KBP_VERSION, true );
		...
		wp_register_script( 'kadence-blocks-pro-splide-init', KBP_URL . 'dist/assets/js/kb-splide-init.min.js', array( 'kadence-splide' ), KBP_VERSION, true );
	}
	/**
	...
	public function render_post_block( $attributes ) {
		...
		if ( ( 'masonry' === $layout || 'grid' === $layout ) && isset( $attributes['displayFilter'] ) && true === $attributes['displayFilter'] ) {
			...
			$this->enqueue_style( 'kadence-splide' );
	...
	public function output_css( $attributes, $unique_id ) {
		...
		if ( isset( $attributes['rowGap'] ) && 'masonry' === $layout ) {
			$css->set_selector( '.kt-post-loop' . $unique_id . ' .kt-post-grid-layout-masonry-wrap .kt-post-masonry-item' );
			$css->add_property( 'padding-bottom', $attributes['rowGap'] / 2 . ( ! empty( $attributes['rowGapUnit'] ) ? $attributes['rowGapUnit'] : 'px' ) );
		}
		...
		if ( isset( $attributes['backgroundColor'] ) || isset( $attributes['borderColor'] ) || isset( $attributes['borderWidth'] ) || isset( $attributes['borderRadius'] ) || ( isset( $attributes['displayShadow'] ) && true == $attributes['displayShadow'] ) ) {
			...
			if ( isset( $attributes['borderRadius'] ) && is_array( $attributes['borderRadius'] ) ) {
				...
				if ( is_numeric( $attributes['borderRadius'][0] ) ) {
					$css->add_property( 'border-top-right-radius', $attributes['borderRadius'][0] . 'px' );
				}
				if ( is_numeric( $attributes['borderRadius'][1] ) ) {
					$css->add_property( 'border-bottom-right-radius', $attributes['borderRadius'][1] . 'px' );
				}
				if ( is_numeric( $attributes['borderRadius'][2] ) ) {
					$css->add_property( 'border-bottom-left-radius', $attributes['borderRadius'][2] . 'px' );
				}
				if ( is_numeric( $attributes['borderRadius'][3] ) ) {
					$css->add_property( 'border-top-left-radius', $attributes['borderRadius'][3] . 'px' );
				}
			}
